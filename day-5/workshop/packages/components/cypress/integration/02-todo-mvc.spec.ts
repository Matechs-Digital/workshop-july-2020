describe("01-go-to-google", () => {
  ...
  it("should complete task-a", () => {
    cy.get("label").contains("task-a").parent().find(".toggle").click();
  });
  ...
  it("should task-a should not exist", () => {
    cy.get(".todo-list").contains("task-a").should("not.exist");
  });
  ...
  it("should task-b should be visible", () => {
    cy.get(".todo-list").contains("task-b").should("be.visible");
  });
